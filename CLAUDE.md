...
**`tests/e2e/stdio.spec.ts` is NOT part of that surface: no `testMatch` in either Playwright config covers it, so it has never executed** (#382). It is listed here only so nobody counts it as coverage. Two things hide it, and both are worth knowing before adding a spec file: `knip.json` declares `tests/e2e/**/*.spec.ts` as an ENTRY pattern, so the blocking dead-code gate treats it as a root rather than as dead code; and this very paragraph used to describe it as live.

NOTE (#366): `tests/e2e/suites/*.ts` used to be described here as the per-domain home. Nothing ever executed those files either (no call site, and no `testMatch` covered them), so they were removed. That sweep missed `stdio.spec.ts` because it ends in `.spec.ts` and so was shielded by the knip entry pattern above.

So: add E2E coverage to `docker-all-tools.e2e.spec.ts`, and **confirm it is collected** with `npx playwright test --list --config playwright.config.docker.ts`. A spec file that no config matches is silently inert, and this repo has now been caught by that twice.
