...
**Actual MCP Server** bridges AI assistants with [Actual Budget](https://actualbudget.org/) via the Model Context Protocol (MCP), exposing **75 tools** for conversational financial management. Supports two transports: **HTTP** (for LibreChat/LobeChat/multi-user deployments) and **stdio** (for Claude Desktop/Claude Code local use; pass the `--stdio` flag).
...
npm run verify-tools            # Step 2: all 75 tools registered (reads dist/)
...
ActualMCPConnection (src/lib/ActualMCPConnection.ts)
    ↓
ActualToolsManager (75 tools, Zod validation, dispatch) at src/actualToolsManager.ts
    ↓
...
**Every tool declares MCP annotations, and they are HINTS, never a guard (#379).** `src/lib/tool-annotations.ts` classifies all 75 tools on the four fields the MCP spec defines, and `src/lib/tool-list-entry.ts` attaches them to every `tools/list` entry. Two things to know before touching this:
