...
So a tool must never wrap an `adapter.*` call in a session of its own, and code already inside a session callback must reach the API through the raw functions, not back through `adapter.*`. Exactly five tool files therefore statically import `@actual-app/api`, for two sanctioned reasons:
...
**Where a read-then-write guard belongs (#371).** Three tools briefly carried one in the tool layer (`accounts_close`, `accounts_reopen`, `budgets_holdForNextMonth`, from #355/#357/#358) and it was moved into the adapter. The single-cycle property never required the raw api: `adapter.mergePayees` and `adapter.deletePayee` do the same read-decide-write inside one `queueWriteOperation`. Doing it in the adapter keeps `retry` on the reads, keeps ONE observability call site, and leaves no unguarded `adapter.*` method for a future caller to reach for. Prefer the adapter; reach for this list's pattern only when a tool genuinely needs several raw calls in one cycle.
