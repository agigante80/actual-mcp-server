...
- `accounts_close.ts`, `accounts_reopen.ts`, `budgets_holdForNextMonth.ts`: added by the #350 write-effect work (#355, #357, #358). Same `withWriteSession` pattern as the first group, but for a read, write and RE-READ in one cycle: each verifies the effect it claims rather than trusting the call to have returned. Note the cost, tracked in #368: a tool on this pattern no longer gets the adapter's `retry` or its per-tool observability counter.
