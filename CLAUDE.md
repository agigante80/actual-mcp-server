...
So a tool must never wrap an `adapter.*` call in a session of its own, and code already inside a session callback must reach the API through the raw functions, not back through `adapter.*`. Exactly EIGHT tool files therefore statically import `@actual-app/api`, for three sanctioned reasons:
...
- `accounts_close.ts`, `accounts_reopen.ts`, `budgets_holdForNextMonth.ts`: added by the #350 write-effect work (#355, #357, #358). Same `withWriteSession` pattern as the first group, but for a read, write and RE-READ in one cycle: each verifies the effect it claims rather than trusting the call to have returned. Note the cost, tracked in #368: a tool on this pattern no longer gets the adapter's `retry`. The per-tool observability counter is not lost, because these tools increment it themselves.
