...
└── docker-all-tools.e2e.spec.ts      ← Comprehensive coverage of all 75 tools; the ONLY
...
| `EXPECTED_TOOL_COUNT` | `75` | Expected tool count (must match `actualToolsManager.ts`) |
...
**Purpose:** Comprehensive named tests for all 75 tools: success paths plus error and negative paths.
