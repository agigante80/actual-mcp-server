...
**Actual MCP Server** bridges AI assistants with [Actual Budget](https://actualbudget.org/) via the Model Context Protocol (MCP), providing **74 tools** for conversational financial management. Supports two transports: **HTTP** (LibreChat/LobeChat/multi-user) and **stdio** (Claude Desktop/Claude Code).

**Tech Stack**: TypeScript (NodeNext/ESM), Node.js 22+, `@actual-app/api` v26, `@modelcontextprotocol/sdk`, Express 5, Zod v4, Playwright

**Current Status**: Production-ready, 74 tools implemented
...
├── actualToolsManager.ts       # Tool registry (74 tools in IMPLEMENTED_TOOLS array), dispatch, validation
├── auth/
...
# Tool Management
npm run verify-tools            # Verify all 74 tools are correctly registered
...
**Version:** 0.16.2  
**Tool Count:** 74 (verified LibreChat-compatible)
