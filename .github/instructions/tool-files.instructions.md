...
- **#371: prefer putting a read-then-write guard in the adapter, not the tool.** The single-cycle property does not require the raw api (`adapter.mergePayees` proves it), and the adapter keeps `retry` and one observability call site
