...
/**
 * Convert Actual's grouped split representation into posting rows. A normal
 * transaction appears once. A split contributes its children and never its parent.
 */
...
export function summarizeAccountFlow(snapshot: FinancialAnalysisSnapshot, input: AccountFlowInput) {
  ...
  const openingBalance = input.accountIds.reduce(
    (sum, id) => sum + integer(snapshot.openingBalances?.[id] ?? 0, `Opening balance for ${id}`),
    0,
  );
  const closingBalance = input.accountIds.reduce(
  ...
  return {
    ...
    accounts: input.accountIds.map(id => ({ id, name: maps.accounts.get(id)?.name ?? id })),
    openingBalance,
    closingBalance,
    balanceChange,
    external: {
