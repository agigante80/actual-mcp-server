...
- `[redacted]/dev/actual-mcp-server/package.json` - Zod version and overrides
- `[redacted]/dev/actual-mcp-server/Dockerfile` - Post-install Zod 3.x enforcement
- `[redacted]/dev/actual-mcp-server/src/lib/ActualMCPConnection.ts` - Uses zodToJsonSchema
- `[redacted]/dev/actual-mcp-server/src/index.ts` - Tool schema extraction
