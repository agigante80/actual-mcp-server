...
| `OIDC_JWKS_URI` | url string | (none) | No | no | schema | `httpServer.ts` via `resolveJwksSource` | #462 direct JWKS URL bypassing discovery (Cloudflare Access `.../cdn-cgi/access/certs`); validated by `resolveDirectJwksUri` (https, no credentials, no query or fragment, same origin or `OIDC_JWKS_TRUSTED_HOSTS`); refused under `AUTH_PROVIDER=none` and with `AUTH_BUDGET_ACL_IDENTITY_SOURCE=userinfo` |
| `OIDC_TOKEN_SOURCE` | enum `authorization`, `cf-access-jwt-assertion` | `authorization` | No | no | schema | `httpServer.ts` via `createAssertionPromotionMiddleware` | #462 which header carries the bearer JWT; `cf-access-jwt-assertion` promotes Cloudflare's forwarded Access JWT into `Authorization` before verification and never consults the original bearer; refused under `AUTH_PROVIDER=none`, with non-empty `OIDC_SCOPES`, and with `AUTH_BUDGET_ACL_IDENTITY_SOURCE=userinfo` |
...
| `OIDC_SCOPES` | csv string | (none) | No | no | schema | config | Comma-separated required (enforced) scopes, also advertised; leave empty when IdP JWTs lack scope claim |
| `OIDC_SCOPES_SUPPORTED` | csv string | (none) | No | no | schema | config | Comma-separated scopes advertised in discovery (`scopes_supported`) but not enforced (#472) |
