...
│   ├── generated_tools.smoke.test.js      # All 74 tools: stub adapter + correctness assertions
...
│   ├── docker-all-tools.e2e.spec.ts       # All-tools Docker E2E (74 tools)
...
- ✅ Real tool execution (**all 74 tools at 100% coverage**)
...
| `generated_tools.smoke.test.js` | All 74 tools: stub adapter, call succeeds, response shape correct |
...
**Coverage**: 74/74 tools smoke-validated (offline, stub adapter). 60+ negative-path assertions across 11+ tool schemas.
...
**Status**: Fully operational. `docker-all-tools.e2e.spec.ts` covers all 74 tools end-to-end.
...
**Purpose**: Smoke test all 74 tools
...
- **Unit Tests**: schema/shape smoke tests + 23 negative-path assertions across 74 tools
...
- **Live Integration**: 74/74 tools called against real budget (all delete tools are named tests in `tests/manual/tests/`)
...
| Integration Tests | 74/74 tools (live) | Maintain | Medium |
| E2E Tests | All 74 tools (Docker) | Maintain | Medium |
...
```
                    ...
                    /             \
                  /   Level 5:      \
                /   Full E2E Tests    \    ← All 74 tools + Error scenarios
              /     (Docker Stack)      \
            /                              \
...
**Coverage:**
- ✅ All 74 tools registered correctly
...
- `tests/unit/generated_tools.smoke.test.js` - All 74 tools smoke validation
...
| `generated_tools.smoke.test.js` | All 74 tools: stub adapter, `call()` succeeds, response shape verified per-tool | 74 + shape checks |
...
**Coverage:**
- ✅ All 74 tools: stub invocation + response-shape assertion
...
| List Tools | ✅ Returns 74 tools | ❌ Timeout |
...
- ✅ **ALL 74 tools execution (100% coverage)**
...
| 3 | List all tools | ✅ 74 tools returned | ❌ Timeout, server error |
...
The authoritative per-domain breakdown lives in `tests/e2e/docker-all-tools.e2e.spec.ts` (describe block `Docker E2E - ALL 74 TOOLS`): it exercises all 74 tools plus error scenarios. The per-category counts are not duplicated here, because a hand-maintained copy drifts.

**Success Criteria:**
- All 74 tools execute successfully
...
**Purpose:** Test all 74 tools with real Actual Budget data  
...
#### FULL Level (74 tools, 100% coverage)
...
**Pre-Release (Manual):**
```bash
# Full manual integration test with all 74 tools
...
| **Level 2:** Unit Tests | 74/74 tools (stub), 23 schema assertions | Maintain + grow | ✅ Good |
| **Level 3:** Live Integration | 74/74 tools called | 74/74 | ✅ Maintain |
...
| **Level 6:** Manual Full | 100% (74/74 tools) | 100% | ✅ Maintain |
...
2. ✅ **Completed:** Unit test suite (~52 files), 74-tool smoke, 23 negative-path assertions
